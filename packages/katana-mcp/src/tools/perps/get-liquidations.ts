...
export function registerGetPerpsLiquidations(server: McpServer) {
  server.registerTool(
    "get_perps_liquidations",
    {
      description:
        "Get recent liquidation data for a perpetual futures market. Unlike trades, liquidations do not contribute to volume or appear in tickers/candles.",
      inputSchema: {
        ...
        limit: z.coerce.number().optional().describe("Max liquidations to return (1-1000, default 50)"),
        network: z
    ...
    async ({ market, start, end, limit, network }) => {
      try {
        ...
        const data = await perpsGet("/v1/liquidations", params, network as NetworkName);
        return {
