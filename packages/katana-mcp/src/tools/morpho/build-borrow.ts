...
const inputSchema = {
  ...
  amount: z
    .string()
    .describe("Amount of loan token to borrow (human-readable, e.g. '500')"),
  onBehalf: z
    .string()
    .regex(/^0x[a-fA-F0-9]{40}$/, "Must be a valid Ethereum address")
    .describe("Address that owns the collateral and will owe the debt"),
  receiver: z
    .string()
    .regex(/^0x[a-fA-F0-9]{40}$/, "Must be a valid Ethereum address")
    .describe("Address to receive the borrowed tokens"),
  network: z
...
export function registerBuildBorrow(server: McpServer) {
  server.registerTool(
    "build_morpho_borrow",
    {
      description:
    ...
    async ({ marketId, amount, onBehalf, receiver, network }) => {
      ...
      let collateralSymbol = "N/A";
      try {
        collateralSymbol = await client.readContract({
          address: params.collateralToken as Address,
          abi: erc20Abi,
          functionName: "symbol",
        });
      } catch {}
      ...
      const tx = {
        ...
        value: "0",
        chainId: chain.id,
        description: `Borrow ${amount} ${loanSymbol} from Morpho market (collateral: ${collateralSymbol})`,
        note: `User must have sufficient ${collateralSymbol} collateral deposited first. LLTV: ${Number(params.lltv) / 1e16}%. Monitor health factor to avoid liquidation.`,
